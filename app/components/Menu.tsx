...
const Menu = () => {
  ...
  useGSAP(() => {
    gsap.timeline({
      ...
      .to('#m-left-leaf', { y: -200, ease: 'none' }, 0)
      .to('#m-right-leaf', { y: 200, ease: 'none' }, 0)
  })
    ...
    <section id="menu" aria-labelledby='menu-heading'>
      ...
      <nav className="cocktail-tabs" aria-label='Cocktail Navigation'>
        {sliderLists.map((cocktail, index) => {
          ...
          return (
            <button key={cocktail.id} className={`${isActive ? 'text-white border-white' : 'text-white/50 border-white/50'} cocktail-tab-btn`} onClick={() => setCurrentIndex(index)}>
              {cocktail.name}
            </button>
          )
        })}
